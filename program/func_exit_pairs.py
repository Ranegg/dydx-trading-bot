from constants import CLOSE_AT_ZSCORE_CROSS, STOP_LOSS_PERCENTAGE
...
def manage_trade_exits(client):
  ...
  # Find out the function of this
  live_orders = []
  for x in all_exc_pos:
    live_orders.append({'market': x['market'], 'side': x['side'], 'entryPrice': float(x['entryPrice'])})
  ...
  for x in live_orders:
    ...
    oracle_prices.append({'market': y['market'], 'indexPrice': float(y['indexPrice'])})
  ...
  for position in open_positions_dict:
    ...
    is_closeByZScore = False
    is_closeByStopLoss = False
    ...
    if CLOSE_AT_ZSCORE_CROSS:
      ...
      if z_score_level_check and z_score_cross_check:
        ...
        is_closeByZScore = True
        send_message("Closing pair due to ZScore cross")
        print("Closing pair due to ZScore cross")
    ...
    # Stop Loss - Percentage defined in constants
    for order in live_orders:
      ...
      for order_current_price in oracle_prices:
        if order_current_price['market'] == order['market']:
          # If Order Long and current price <= entry price * STOP_LOSS_PERCENTAGE / 100 -> close the order, same for short
          if order_side == 'LONG' and order_current_price['indexPrice'] <= order_entry_price - (order_entry_price * STOP_LOSS_PERCENTAGE / 100):
            is_closeByStopLoss = True
            break
          elif order_side == 'SHORT' and order_current_price['indexPrice'] >= order_entry_price + (order_entry_price * STOP_LOSS_PERCENTAGE / 100):
            is_closeByStopLoss = True
            break
      if is_closeByStopLoss == True:
        send_message("Closing pair due to Stop Loss trigger")
        print("Closing pair due to Stop Loss trigger")
        break

    # Close positions if triggered
    if is_closeByZScore or is_closeByStopLoss:
